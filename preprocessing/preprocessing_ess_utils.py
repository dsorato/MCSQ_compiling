...
def instruction_recognition_german(text,country_language):
	...
	regex= r"^(?P<readoutstatements>)JEDES\sSTATEMENT\sVORLESEN\s(?P<andcode>)UND\sKODIEREN"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<please>)(bitte)?\s?(?P<use>)(Verwenden|benutzen|nutzen)\sSie\s(?P<again>)dazu wieder\s(?P<the>)(die)?\s?(?P<card>)(Karte|liste)"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches and '?' not in text_aux and ':' not in text_aux and '...' not in text_aux and 'ablehnen' not in text_aux and 'sagen Sie mir' not in text_aux:
		return True
...
def instruction_recognition_french(text,country_language):
	...
	regex= r"(?P<one>)UNE\s(?P<answer>)(REPONSE|RÉPONSE)(?P<forline>)par\sligne"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<multiple>)(PLUSIEURS|Plusieures)\s(?P<answers>)(reponses|réponses)\s(?P<possible>)possibles"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
...
def instruction_recognition_czech(text,country_language):
	...
	regex= r"^(?P<respondent>)RESPONDENKY\s(?P<malefemale>)(ŽENY|MUŽI)"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<read>)(ČTĚTE|ČTETE|PŘEČTĚTE)\s(?P<out>)NAHLAS\s(?P<each>)(KAŽDÝ)?\s?(?P<utterance>)VÝROK"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
...
def instruction_recognition_portuguese(text,country_language):
	...
	regex= r"^(?P<show>)(mostrar|manter)\s(?P<again>)(novamente)?\s?(?P<card>)cartão"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True

	regex= r"^(?P<show>)(mostrando|utilizando)\s(?P<again>)ainda\s(?P<this>)este\s(?P<card>)cartão"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<please>)(por favor)?\s?(?P<use>)utilize\s(?P<this>)este\s(?P<card>)cartão"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True

	regex= r"^(?P<use>)(utilize\s|use\s)(?P<please>)(por\sfavor)?\s?(?P<this>)este\s(?P<card>)cartão"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<please>)(por favor)?\s?(?P<choose>)escolha\s(?P<youranswer>)respostas\s(?P<fromthis>)(a\spartir\s)?deste\s(?P<card>)cartão"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
	...
	regex= r"^(?P<codeormark>)(codificar|assinalar|registrar)\s(?P<only>)(só|apenas)?\s?(?P<one>)uma\s(?P<answer>)resposta"
	matches = re.search(regex, text, re.IGNORECASE)
	if matches:
		return True
...
def instruction_recognition_catalan_spanish(text,country_language):
	...
	if 'CAT' in country_language:
		...
		regex= r"^(?P<use>)(utilice|use)\s(?P<again>)(otra vez)?\s?(?P<this>)(esta)?\s?(?P<card>)tarjeta(?P<please>)(por favor)?\s?(?P<toindicate>)para\sindicarlo"
		matches = re.search(regex, text, re.IGNORECASE)
